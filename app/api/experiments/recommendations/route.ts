...
Tugas kamu:
Berikan TEPAT 3 rekomendasi tindakan bisnis yang SPESIFIK, BERBEDA SATU SAMA LAIN, dan langsung bisa dilaksanakan pedagang ini HARI INI. Setiap rekomendasi WAJIB mengacu pada nama produk nyata di atas.
...
[
  {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        recommendations = parsed.slice(0, 3).map((r: any, idx: number) => ({
          id: `rec-${Date.now()}-${idx}`,
          productName: r.productName || productSummaries[0]?.name || 'Produk Anda',
      ...
      // Fallback: buat rekomendasi berbasis logika dari data produk nyata
      recommendations = productSummaries.slice(0, 3).map((p: any, idx: number) => {
        ...
        if (margin <= 0 && cost > 0) {
          ...
          const nextProd = productSummaries[(idx + 1) % productSummaries.length];
